  ...
  function subscribeNow(session, config, next) {
    sequentialChannelOperations.push((done) => {
      ...
      vhost.getChannel((err, channel) => {
        ...
        const consumePromise = promisify(channel.consume).bind(channel)

        consumePromise(config.source, onMessage, config.options)
          .then((response) => {
            debug('Setting up subscription: %s. %s', config.source, response.consumerTag)
          ...
          .catch((error) => {
            debug('Error subscribing to %s using channel: %s. %s', config.source, channel._rascal_id, error.message);
            removeErrorHandlers();
