...
function Subscription(broker, vhost, subscriptionConfig, counter) {
  ...
  this.subscribe = function (overrides, next) {
    ...
    console.log({
      msg: 'Subscribtion.subscribe session',
      data: {
        session,
      }
    })
  ...
  function subscribeLater(session, config) {
    ...
    session.on('newListener', (event) => {
      console.log({
        msg: 'Subscription.subscribeLater#newListener',
        data: {
          event,
        }
      })
      ...
      subscribeNow(session, config, (err) => {
        console.log({
          msg: 'Subscription.subscribeLater#newListener#subscribeNow#cb',
          data: {
            err,
          }
        })
  ...
  function subscribeNow(session, config, next) {
    ...
    sequentialChannelOperations.push((done) => {
      ...
      vhost.getChannel((err, channel) => {
        console.log({
          msg: 'Subscription.subscribeNow#vhost.getChannel',
          data: {
            err,
            channel,
          }
        ...
        if (err) {
          debug('There was an error getting channel');
          return done(err);
        }

        if (!channel) {
          debug('No channel; returning');
          return done();
        }

        if (config.prefetch) {
          debug('Configuring prefetch')
          channel.prefetch(config.prefetch);
        }

        debug('Create removeErrorHandlers')
        const removeErrorHandlers = attachErrorHandlers(channel, session, config);

        debug('Create onMessage')
        const onMessage = _onMessage.bind(null, session, config, removeErrorHandlers);

        debug('Before channel.consume: %s', config.source)
        try {
          const channelConsumeResult = channel.consume(config.source, onMessage, config.options, (err, response) => {
            debug('Entered channel.consume: %s', config.source)
            console.log({
              msg: 'Subscription.subscribeNow#channel.consume',
              data: {
                err,
                response,
              }
            ...
            debug('Before session._open: %s', config.source)
            session._open(channel, response.consumerTag, (err) => {
              debug('Inside session._open : %s', config.source)
              console.log({
                msg: 'Subscription#subscribeNow#session._open',
                data: {
                  err,
                  channel,
                  response,
              ...
              if (err) {
                debug('Error opening opening session: %s. %s', channel._rascal_id, err.message);
          ...
          console.log({
            msg: 'channelConsumeResult type',
            data: {
              channelConsumeResult
            }
          })

          debug('Finished channel.consume: %s', config.source)
        } catch (error) {
          console.error({
            msg: 'caught error on channel.consume',
            data: {
              error,
            }
          })
